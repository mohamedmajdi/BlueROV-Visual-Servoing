...
class MyPythonNode(Node):
    ...
    def timer_callback(self):
        ...
        if self.set_mode[0]:  # commands sent inside joyCallback()
            ...
            # self.setOverrideRCIN(1500, 1500, 1500, self.Correction_yaw, self.Correction_surge, self.obj_sway_correction)
    ...
    # def timer_callback(self):
        # if self.pinger_distance is not None:
        #     self.object_avoidance()
    ...
    def distance_stable(self,window_size = 10, threshold = 2.0,tolerance = 0.5):
        ...
        # return abs(average - self.object_distance) < tolerance
    ...
    def object_avoidance(self):
        ...
        if self.object_state == "Free":
            ...
            if self.distance_stable(window_size=3):
            ...
            # elif (self.yaw_desired - np.deg2rad(5)) <= self.current_yaw <= (self.yaw_desired + np.deg2rad(5)):
            #     self.get_logger().info("in")
            self.yaw_desired += np.deg2rad(-1)
            # self.obj_sway_correction = 1530
